...
import type { Alternative, ArvoreMateria, ArvoreTopico, ArvoreSubtopico } from '@/lib/questoes'
import RichTextArea from '@/components/RichTextArea'
import Nav from '@/components/Nav'
import { ArrowLeft } from 'lucide-react'

const EMPTY_ALTS: Alternative[] = [
  { letter: 'A', text: '' },
  { letter: 'B', text: '' },
  { letter: 'C', text: '' },
  { letter: 'D', text: '' },
...
export default function NovaQuestaoPage() {
  ...
  // Carrega matérias
  useEffect(() => {
    dbQuery<ArvoreMateria>('arvore_materias', { order: 'ordem.asc' }).then(({ data }) => {
      setMaterias(data || [])
    })
  }, [])

  // Carrega tópicos quando matéria muda
  useEffect(() => {
  ...
  // Carrega subtópicos quando tópico muda
  useEffect(() => {
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    const { data, error: err } = await dbInsert('questions', body, true)
    if (err) {
      setError(err)
      setSaving(false)
    } else {
      router.push(`/questoes/${data?.[0]?.id || ''}`)
    }
  }
    ...
    <div style={{ padding: '0 0 80px' }}>
      <div style={{ position: 'sticky', top: 0, background: '#F7F6F3', zIndex: 10, padding: '16px 16px 14px', borderBottom: '1px solid #e2e8f0' }}>
        <div style={{ maxWidth: 800, margin: '0 auto', display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div style={{ fontSize: 18, fontWeight: 700, letterSpacing: '-0.3px' }}>Nova questão</div>
        </div>
      </div>
        ...
        <div style={{ maxWidth: 800, margin: '0 auto', padding: '20px 16px', display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{ background: 'white', border: '1.5px solid #e2e8f0', borderRadius: 12, padding: 20 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              {/* Árvore: 3 selects cascateados */}
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
                <div>
                  <label style={labelStyle}>Matéria *</label>
                  <select value={materiaId} onChange={e => setMateriaId(e.target.value)} required style={selectStyle}>
