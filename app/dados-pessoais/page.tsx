  ...
  return (
    <div style={{ paddingBottom: 80 }}>
      ...
      <div style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        <div className="card" style={{ textAlign: 'center', padding: 24 }}>
          <div style={{ marginBottom: 14 }}>
            {fotoPreview ? (
              ...
              <div style={{ width: 80, height: 80, borderRadius: '50%', background: '#EEEDFE', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 28, fontWeight: 600, color: '#3C3489', margin: '0 auto' }}>
                {meuPerfil?.nome?.split(' ').map((w: string) => w[0]).slice(0, 2).join('') || '?'}
              </div>
            )}
          </div>
          <label style={{ display: 'inline-block', background: '#EEEDFE', color: '#534AB7', padding: '8px 16px', borderRadius: 10, fontSize: 13, fontWeight: 500, cursor: 'pointer' }}>
            📷 Alterar foto
        ...
        {msg && <div style={{ fontSize: 13, color: msg.startsWith('✅') ? '#1D9E75' : '#E24B4A', textAlign: 'center' }}>{msg}</div>}
